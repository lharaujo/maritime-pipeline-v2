...
# We import simple utils, ignoring the Modal decorators in the original file effectively
...
def ensure_ports_exist(con):
    ...
    try:
        ...
        # We need to adapt the scraping logic to return a DataFrame instead of saving parquet
        # For simplicity in this refactor, let's assume we scrape and load to MD
        # In a real scenario, you'd call your scrape function here.
        ...
        raise RuntimeError("Ports table missing. Please run initial bootstrap locally or port scraping logic.")
...
def fetch_and_filter_ais(year, month, day, tree, port_locodes, port_names):
    ...
    print(f"📦 Data downloaded. Parsing CSV...")
    ...
    filtered_ais = filtered_ais.with_columns([
        ...
        pl.Series("dep_time", filtered_ais["base_date_time"].str.to_datetime(strict=False)) # Renaming base_date_time
...
def main():
    ...
    # Determine date
    # Simple logic: Run for yesterday
    target_date = datetime.utcnow() - timedelta(days=1)
    
    # Check environment override
    # logic to parse args could go here
    ...
    if df_filtered is not None and not df_filtered.is_empty():
        ...
        # Insert Data
        # DuckDB generic relationship insert
